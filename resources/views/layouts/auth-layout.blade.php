...
<div class="relative overflow-hidden">
  <div class="mx-auto max-w-screen-md py-12 px-4 sm:px-6 md:max-w-screen-xl md:py-20 lg:py-32 md:px-8">
    <div class="md:pe-8 md:w-1/2 xl:pe-0 xl:w-5/12">
      ...
      <div class="mt-8 grid">
        <a href="{{ route('redirectToGoogle') }}" class="py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 focus:outline-none focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-800 dark:focus:bg-neutral-800">
      ...
      <!-- Form -->
      <form action="{{ $action }}" class="space-y-2 px-6" method='post'>
        @csrf
        {{ $slot }}
        <div class="pt-4">
          <button type="submit" class="w-full rounded border border-orange-300 bg-orange-500 py-2.5 px-3 text-white font-medium font-sans transition hover:bg-opacity-90">
            {{ $submitButtonMessage }}
          </button>
        </div>
      </form>
      <div class="space-y-4 mt-4">
        <p class="text-sm text-center text-gray-500 dark:text-dark-6 pt-6">
          <span class="pr-0.5">{{ $authBackMessage }}</span>
          <a href="{{ $authBackLink }}" class="text-indigo-500 hover:underline">
            {{ $authBackLinkText }}
          </a>
        </p>
      </div>
  ...
  <div class="hidden md:block md:absolute md:top-0 md:start-1/2 md:end-0 h-full bg-local bg-no-repeat bg-center bg-cover" style="background-image:url('{{asset("images/frontend/login.jpeg")}}')"></div>
  <!-- End Col -->
</div>
<!-- End Hero -->
@endsection
