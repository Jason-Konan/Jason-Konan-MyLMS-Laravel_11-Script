                ...
                <div class="table-responsive">
                  <table id="example3" class="display" style="min-width: 845px">
                    ...
                    <tbody>
                      @forelse ($event as $e)
                      <tr>
                        <td><img src="{{asset('public/uploads/events/'.$e->image)}}" class="w-100" height="50"></td>
                        <td><strong>{{$e->title}}</strong></td>
                        <td><strong>{{$e->topic}}</strong></td>
                        <td>{{$e->location}}</td>
                        <td>{{ \Carbon\Carbon::parse($e->date)->format('j F, Y, l') }}</td>
                        <td>
                          <a href="{{route('event.edit', $e->id)}}" class="btn btn-sm btn-primary" title="Edit"><i class="la la-pencil"></i></a>
                          <a href=";" class="btn btn-sm btn-danger" title="Delete" onclick="$('#form{{$e->id}}').submit()"><i class="la la-trash-o"></i></a>
                          <form id="form{{$e->id}}" action="{{route('event.destroy', $e->id)}}" method="post">
                            @csrf
                            @method('DELETE')
                          </form>
                        </td>
