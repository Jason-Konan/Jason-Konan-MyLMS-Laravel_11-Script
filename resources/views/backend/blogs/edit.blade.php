<x-admin-layout title="Edit your post">
  ...
  <div class="max-w-2xl mx-auto p-4">
    <form action="{{ route('admin.blog.update', $blog) }}" method="POST" enctype="multipart/form-data">
      ...
      <x-input type="text" id="title" name="title" label="Title" placeholder="Blog One" value="{{$blog->title ?? (old('title')??'')}}" />

      <x-input type="text" id="slug" name="slug" label="Slug" placeholder="blog-one" value="{{$blog->slug ?? (old('slug')??'')}}" />
      ...
      <div class="mb-6">
        ...
        <select name="category" id="category" class="p-2 rounded-md border-gray-300 border w-full">
          <option class="text-gray-500" value="{{ $blog->category_for_blog_id ?? '' }}">
            {{ $blog->category_for_blog->name ?? 'Select a category' }}
          </option>
      ...
      <div class="mb-6">
        ...
        <textarea id="tinymce" name="content" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-indigo-500" rows="5">{!! $blog->content !!}</textarea>
        @error('content')
        <p class="mt-2 text-red-500">{{ $message }}</p>
        @enderror
      </div>
