<x-admin-layout title="Create Categories For Blogs">
  <div class="flex items-center justify-center bg-blue-600 dark:bg-sky-500/50 h-64">
    <h1 class="text-3xl lg:text-4xl text-white font-bold mb-5">{{__("Create Category")}} </h1>

  </div>

  <section class="max-w-6xl mx-auto px-4 py-8">
    <div class="flex flex-col items-center justify-center gap-6 py-4">
      <form class="space-y-4 w-full" action="{{ route('store.blogCategory') }}" method="post" enctype="multipart/form-data">

        @csrf
        <x-input name="image" label="Category Image" type="file" id="name" />
        <div class="mb-4">
          <x-input name="name" label="Name" type="text" id="name" placeholder="Category Name" />

        </div>
        <div class="mb-6">
          <label for="description" class="block text-lg font-medium text-gray-800 mb-1"> {{__("Description")}} </label>
          <textarea name="description" class="form-textarea w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-indigo-500" rows="3" placeholder="Category Description" value="{{old('description')}}"></textarea>
          @error('description')
          ...
          <tbody>
            ...
            <tr class="border">
              ...
              <td class="px-4 py-3 flex items-center justify-center gap-4">
                <a href="{{ route('blogCategory.edit', $category) }}" class="mt-3 px-4 py-2 text-indigo-600  font-semibold rounded-md hover:text-indigo-700"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                  </svg>
                </a>
                <form action="{{ route('destroy.blogCategory', $category) }}" method="post" class="">
                  @method('delete')
                  @csrf
